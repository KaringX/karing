<h1 align="center">
  <img src="./README_assets/img/mascot.jpg" alt="Karing" width="256" />
  <br>
  Karing - 簡單且強大的代理工具
  <br>
</h1>

<h3 align="center">
一個基於 <a href="https://github.com/flutter/flutter">flutter</a> 的 <a href="https://github.com/SagerNet/sing-box">singbox</a> 圖形使用者介面。
</h3>

[English](./README.md) | [简体中文](./README_cn.md) | 繁體中文 | [日本語](./README_ja.md) | [한국어](./README_ko.md) | [Español](./README_es.md) | [Français](./README_fr.md) | [Deutsch](./README_de.md) | [Italiano](./README_it.md) | [Tiếng Việt](./README_vi.md) | [Türkçe](./README_tr.md) | [Русский](./README_ru.md) | [فارسی](./README_fa.md) | [العربية](./README_ar.md) | [Português](./README_pt.md) | [Українська](./README_uk.md) | [Polski](./README_pl.md) | [اردو](./README_ur.md) | [Svenska](./README_sv.md) | [Norsk](./README_no.md) | [Nederlands](./README_nl.md) | [हिन्दी](./README_hi.md) | [Ελληνικά](./README_el.md) | [Dansk](./README_da.md) | [বাংলা](./README_bn.md)

### 注意：Karing 未在任何視頻平台開設過有關 Karing 的任何頻道

## 特點
- 相容 Clash、V2ray/V2fly、Sing-box、Shadowsocks、Sub、Github 訂閱。
  - 完全支援 `clash` 配置，部分支援 `clash.meta` 配置。

- 一套路由規則應用於多個訂閱源，自動選擇高效節點。
- 支援自定義路由規則組和節點組。
  - 為小白用戶定制預設路由規則組 - 開箱即用。
  - 內置 geo-IP、geo-site、ACL 和[其他規則集](https://github.com/KaringX/karing-ruleset/)

- 備份和同步，一次配置多設備同步。
  - 支援 iCloud 同步 [IOS/MacOS]。
  - 支援區域網絡內同步。
  - 支援 WebDAV。
  - 支援導入/導出 ZIP 文件。

- 內置支援[魔改版 sing-box 核心](https://github.com/KaringX/sing-box)。
- 增加新手模式，配置更簡單。
- 計劃支援更多平台。

## 推廣
<details>
<summary>查看所有推廣

### 推薦機場

[狗狗加速 —— 技術流機場 Doggygo VPN](https://1.x31415926.top/redir.html?url=aHR0cHM6Ly93d3cuZGc2LnRvcC8jL3JlZ2lzdGVyP2NvZGU9bEZINGlpOUQ=&i=3eb&t=1723644053)

- 高性能海外機場，海外團隊，無跑路風險
- 專屬連結註冊送 3 天，每天 1G 流量 [免費試用](https://1.x31415926.top/redir.html?url=aHR0cHM6Ly93d3cuZGc2LnRvcC8jL3JlZ2lzdGVyP2NvZGU9bEZINGlpOUQ=&i=3eb&t=1723644053)
- 優惠套餐每月僅需 15.8 元，160G 流量，年付 8 折
- 全球首家支援`Hysteria2` 協議，集群負載均衡設計，高速專線，極低延遲，無視晚高峰，4K 秒開
- 解鎖串流媒體及 ChatGPT

[👉更多機場優惠 每日更新](https://1.x31415926.top/)

</summary>

### 🤝VPN 提供商合作公告
- 👉[聯繫方式與合作形式](https://karing.app/blog/isp/cooperation#for-vpn-providers-from-other-regions)👈
</details>

## 系統要求
- Windows >= 10 (僅限 64 位)
- Android >= 8 (arm64-v8a, armeabi-v7a)
- Linux (僅限 64 位)
- IOS >= 15
- MacOS >= 12 (Intel, Apple Silicon)
- TvOS >= 17

## 安裝
- **IOS/TvOS AppStore**: (搜索關鍵詞：karing vpn)
  - https://apps.apple.com/us/app/karing/id6472431552
- **IOS/TvOS TestFlight**:
  - https://testflight.apple.com/join/RLU59OsJ
- **Android**:
  - [https://karing.app/download](https://karing.app/download)
  - https://github.com/KaringX/karing/releases/latest
  - APKPure https://apkpure.com/p/com.nebula.karing
  - Amazon AppStore https://www.amazon.com/gp/product/B0DJSQDDM8
- **Windows/Macos/Linux**:
  - [https://karing.app/download](https://karing.app/download)
  - https://github.com/KaringX/karing/releases/latest

### 常見問題

> [FAQ|tw](https://karing.app/en/faq/)

## 待辦事項
- 待支援的平台：
  - linux | [linglong](https://linglong.dev/)
- 全球化（多語言）
  - 如果您願意與我們合作支援 Karing 全球化，請聯繫我們 [Telegram](https://t.me/ovowe)

## 截圖

<div align="center">
  <img src="./README_assets/demo/home.png" alt="demo1" width="50%" />
  </br></br>
  <img src="./README_assets/demo/select_server.png" alt="demo2" width="50%" />
    </br></br>
  <img src="./README_assets/demo/connections.png" alt="demo3" width="50%" />
  </br></br>
  <img src="./README_assets/demo/setting.png" alt="demo4" width="50%" />
  </br></br>
  <img src="./README_assets/demo/routing_group.png" alt="demo5" width="50%" />
  </br></br>
  <img src="./README_assets/demo/add_profile_link.png" alt="demo6" width="50%" />
</div>

## 貢獻
[歡迎報告問題！](https://github.com/KaringX/karing/issues)

## 捐贈
![donate](./README_assets/img/donate-usdt.jpg)

## 項目
### 致謝：Karing 基於或受到這些項目的啟發：

- [flutter](https://flutter.dev/): 使構建美觀的移動端及跨平台應用變得輕鬆快捷。
- [singbox](https://sing-box.sagernet.org/): 通用代理平台。
- [Meta-Docs](https://wiki.metacubex.one/config/): Clash.Meta 文檔

### Karing 團隊：
- [Karing](https://karing.app): https://karing.app
- [Clash Mi](https://clashmi.app/): https://clashmi.app/
- [sing-poet](https://github.com/KaringX/sing-poet)

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=KaringX/karing&type=Date)](https://star-history.com/#KaringX/karing&Date)
