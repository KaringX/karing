...
class VPNService {
  ...
  static init() async {
    ...
    if (Platform.isMacOS) {
      _useSystemExtension = await File(
        PathUtils.tunnelServiceSEPath(),
      ).exists();
    }
  ...
  static Future<bool> _prepareConfig() async {
    ...
    config.expired_time = DateTime.now()
  ...
  static Future<void> _prepareRuntimeCoreFiles(
    ...
    if (Platform.isIOS ||
      ...
      var fileLog = File(
        serviceLogFilePath,
      ); //日志文件必须先创建,否则ios及macos下sb会包chown错误
  ...
  static List<SingboxOutboundUrltest> getUrltests(
    Set<String> allOutboundsTags, {
    bool uniTag = true,
    ...
    for (var ul in customGroup.urltests) {
      ...
      urltest.tag = uniTag
          ? ServerManager.getUrltestTagForCustom(ul.remark)
          : ul.remark;
  ...
  static Future<ReturnResultError?> setServer(
    ...
    getOutboundsWithoutUrltest(
      allOutboundsTags,
      allOutbounds,
      testGroupTags,
      sortByLatency: currentServer.groupid == ServerManager.getUrltestGroupId(),
    );
    ...
    List<SingboxOutboundUrltest> urltests = getUrltests(
      allOutboundsTags,
      uniTag: true,
      includeEmpty: false,
    );
  ...
  static Future<ReturnResultError?> setOutbound(
    ...
    var setting = SettingManager.getConfig();
    String? clientSubnet = await GeoipSubnetUtils.subnet(
      setting.regionCode.toLowerCase(),
    );
    if ((exportType == SingboxExportType.karing) &&
      ...
      try {
        ...
        var validation = LocalSingboxConfigUtils.validate(content);
        if (validation != null) {
          return validation;
        }
        var config = Map<String, dynamic>.from(jsonDecode(content));
        var log = config["log"] is Map
            ? Map<String, dynamic>.from(config["log"])
            : <String, dynamic>{};
        var experimental = config["experimental"] is Map
            ? Map<String, dynamic>.from(config["experimental"])
            : <String, dynamic>{};
        var inbounds = config["inbounds"] is List
            ? List<dynamic>.from(config["inbounds"])
            : <dynamic>[];

        log["level"] = SingboxConfigBuilder.logLevel();
        log["output"] = serviceLogFileName;

        var clashapi = experimental["clash_api"] is Map
            ? Map<String, dynamic>.from(experimental["clash_api"])
            : <String, dynamic>{};
        var cachefile = experimental["cache_file"] is Map
            ? Map<String, dynamic>.from(experimental["cache_file"])
            : <String, dynamic>{};
        ...
        await _prepareRuntimeCoreFiles(serviceLogFilePath, cacheDBFilePath);
        return null;
      } catch (err, stacktrace) {
    ...
    List<NetInterfacesInfo> interfaces = await NetworkUtils.getInterfaces(
      ...
      filter: !Platform.isWindows,
    );

    SingboxConfig config = SingboxConfig();
    String externalControllerHost =
        (exportType == SingboxExportType.tvos ||
    ...
    if (PlatformUtils.isPC()) {
      if (setting.proxy.enableCluster) {
        List<dynamic> routes = [];
        var inbounds = await ProxyCluster.inboundsAndRulesFrom(
          allOutboundProxys,
          routes,
        );
  ...
  static Future<void> setDefaultOutbound() async {
    ...
    config.ntp = SingboxConfigBuilder.ntp();
    config.dns = SingboxConfigBuilder.dns(
      false,
