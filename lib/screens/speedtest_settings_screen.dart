...
class SpeedTestSettingsScreen extends LasyRenderingStatefulWidget {
  ...
  const SpeedTestSettingsScreen({super.key});

  @override
  State<SpeedTestSettingsScreen> createState() =>
      _SpeedTestSettingsScreenState();
}

class _SpeedTestSettingsScreenState
    extends LasyRenderingState<SpeedTestSettingsScreen> {
  List _searchedData = [];
  ...
  void _buildData() {
    List<String> list = [];
    for (var item in SettingManager.getConfig().speedTestList) {
      if (!SettingConfig.kSpeedTestList.contains(item)) {
        list.add(item);
      }
    }

    list.addAll(SettingConfig.kSpeedTestList);
    _searchedData = list;
  }
