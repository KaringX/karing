...
class SegmentedButtonEx<T> extends StatefulWidget {
  ...
  /// [ChoiceChip] widgets.
  final List<ButtonSegmentEx<T>> segments;
  ...
  @override
  State<SegmentedButtonEx<T>> createState() => SegmentedButtonExState<T>();
}

/// State for [SegmentedButton].
@visibleForTesting
class SegmentedButtonExState<T> extends State<SegmentedButtonEx<T>> {
  bool get _enabled => widget.onSelectionChanged != null;

  /// Controllers for the [ButtonSegment]s.
  @visibleForTesting
  final Map<ButtonSegmentEx<T>, MaterialStatesController> statesControllers =
      <ButtonSegmentEx<T>, MaterialStatesController>{};

  @override
  void didUpdateWidget(covariant SegmentedButtonEx<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget != widget) {
      statesControllers.removeWhere((
        ButtonSegmentEx<T> segment,
  ...
  Widget build(BuildContext context) {
    ...
    final SegmentedButtonThemeData defaults = _SegmentedButtonDefaultsM3(
      context,
    );
    ...
    final Set<MaterialState> currentState = _enabled
        ? enabledState
        : disabledState;
    ...
    Widget buttonFor(ButtonSegmentEx<T> segment) {
      final Widget label =
    ...
    final BorderSide disabledSide =
        ...
        BorderSide.none;
    final OutlinedBorder enabledBorder = resolvedEnabledBorder.copyWith(
      side: enabledSide,
    );
    final OutlinedBorder disabledBorder = resolvedDisabledBorder.copyWith(
      side: disabledSide,
    );
...
class _SegmentedButtonRenderWidget<T> extends MultiChildRenderObjectWidget {
  ...
  void updateRenderObject(
    ...
    _RenderSegmentedButton<T> renderObject,
  ) {
    renderObject
      ..segments = segments
      ..enabledBorder = enabledBorder
...
class _RenderSegmentedButton<T> extends RenderBox
  ...
  _RenderSegmentedButton({
    required List<ButtonSegmentEx<T>> segments,
  ...
  List<ButtonSegmentEx<T>> get segments => _segments;
  List<ButtonSegmentEx<T>> _segments;
  set segments(List<ButtonSegmentEx<T>> value) {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    while (child != null) {
      final _SegmentedButtonContainerBoxParentData childParentData =
          child.parentData! as _SegmentedButtonContainerBoxParentData;
      final Rect childRect = childParentData.surroundingRect!.outerRect.shift(
        offset,
      );
