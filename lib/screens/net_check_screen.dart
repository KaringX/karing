...
class _NetCheckScreenState extends LasyRenderingState<NetCheckScreen> {
  ...
  Future<bool> _checkConnectivity() async {
    ...
    if (ipv4Ok || (!setting.novice && ipv6Ok)) {
      _netCheckItemConnectivity?.values.add(
        ReturnResult(data: tcontext.NetCheckScreen.connectivityTestOk),
      );
    } else {
      _netCheckItemConnectivity?.values.add(
        ReturnResult(
  ...
  Future<bool> _checkOutbound() async {
    ...
    if (NetworkUtils.isDomain(domain, false)) {
      ...
      if (resultDns.error == null) {
        ...
        if (err == null) {
          _netCheckItemOutbound?.values.add(
            ReturnResult(
          ...
          _netCheckItemOutbound?.values.add(
            ReturnResult(
              error: ReturnResultError(
                tcontext.NetCheckScreen.dnsFailed(p1: domain, p2: tag, p3: err),
              ),
            ),
          );
          return false;
    ...
    if (result.error == null) {
      _netCheckItemOutbound?.values.add(
        ReturnResult(data: tcontext.NetCheckScreen.outboundOk(p: name)),
      );
    } else {
      _netCheckItemOutbound?.values.add(
        ReturnResult(
          error: ReturnResultError(
            tcontext.NetCheckScreen.outboundFailed(
              p1: name,
              p2: result.error!.message,
            ),
          ),
        ),
      );
  ...
  Future<bool> _checkDnsQuery(
    NetCheckItemWarpper wrapper,
    List<DNSType> dnsTypes,
  ) async {
    final tcontext = Translations.of(context);
    ...
    for (var dnsType in dnsTypes) {
      ...
      ReturnResult<int> result = await ServerManager.testDNSConnectLatency(
        dnsAddress,
        detour,
        null,
      );
  ...
  Future<bool> _checkDomainDnsQuery() async {
    ...
    List<String> dnsResult = await NetworkUtils.dnsLookup(
      _domainAndPort.item1,
    ...
    if (result.error == null) {
      ...
      if (err == null) {
        ...
        _netCheckItemDomainDNSQuery?.values.add(
          ReturnResult(
            error: ReturnResultError(
              tcontext.NetCheckScreen.dnsFailed(
                p1: _domainAndPort.item1,
                p2: tag,
                p3: err,
              ),
  ...
  Future<bool> _checkRemoteRulesets() async {
    ...
    if (result.error == null) {
      ...
      result.data!.forEach((key, value) {
        if (value == 0) {
          _netCheckItemRemoteRulesets?.values.add(
            ReturnResult(
              error: ReturnResultError(
      ...
      if (_netCheckItemRemoteRulesets!.values.isEmpty) {
        _netCheckItemRemoteRulesets?.values.add(
          ReturnResult(data: tcontext.NetCheckScreen.remoteRulesetsDownloadOk),
        );
      }
    } else {
      _netCheckItemRemoteRulesets?.values.add(
  ...
  Future<bool> _checkHostConnectivity() async {
    ...
    if (NetworkUtils.isDomain(_domainAndPort.item1, false)) {
      ...
      _netCheckItemHostConnectivity?.values.add(
        ReturnResult(
      ...
      if (result.error == null ||
        ...
        _netCheckItemHostConnectivity?.values.add(
          ReturnResult(data: tcontext.NetCheckScreen.hostConnectionOk),
        );
      } else {
        _netCheckItemHostConnectivity?.values.add(
          ReturnResult(
